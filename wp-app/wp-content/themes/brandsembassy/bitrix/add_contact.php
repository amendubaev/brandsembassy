...
if ($id > 1) {
	...
	$additionalBranch = $value['UF_CRM_1557472176436'];
	writeToLog($additionalBranch, '$additionalBranch');
	...
	foreach ($result as $value) {
		if (!in_array($value['ID'], $expertCheck)) {
			...
			writeToLog($post_id, '$post_id');
			writeToLog($bitrix_id, '$bitrix_id');
